"""tests/cv_scoring/test_cv_scoring_synthetic.py — CV 점수 18항목 검증 하니스.

합성 이미지(정답을 아는 입력)로 9 카테고리 18 항목 CV 점수 로직을 4계층에서 검증한다.

계층
----
L1. 순수 매핑 함수(이미지 불필요): 브레이크포인트·스케일 변환의 단조성/경계/왕복.
L2. 단조성: 결함 강도↑ → 해당 점수가 기대 방향(높을수록 좋음 → 감소)으로 이동.
L3. 불변식(전 18항목): 결정론·범위[0,100]·결함 0개 시 오탐 없음.
...
검증 현황 (실측 기반)
--------------------
- VERIFIED(18): 18 항목 전부 합성 결함으로 단조 감소가 실측 확인됨 → L2 단조 테스트 적용.
...
NEEDS_CALIBRATION = sorted(set(ALL_21) - set(VERIFIED_MONOTONIC))
...
class TestL3_Invariants:
    @pytest.mark.parametrize("metric", ALL_21)
