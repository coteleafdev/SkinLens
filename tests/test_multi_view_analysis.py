...
class TestMultiViewMerger:
    ...
    def test_merge_with_max_method(self):
        ...
        # 여드름: 최대값 기반 통합 확인 (소수점 오차 허용)
        assert abs(result["measurements"]["acne_score"] - 50.0) <= 1.0
    ...
    def test_merge_with_missing_lateral(self):
        ...
        # 정면만 사용 (변경 없음, 소수점 오차 허용)
        assert abs(result["measurements"]["eye_wrinkle_score"] - 50.0) <= 1.0
        assert abs(result["measurements"]["melasma_score"] - 60.0) <= 1.0
