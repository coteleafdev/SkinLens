...
def setup_logging(
    ...
    Args:
        ...
        force: True여도 이미 파일 핸들러가 있으면 재설정하지 않음 (무시됨).
        enable_db_logging: True이면 DB 로깅 핸들러 추가.
        mode: 실행 모드 (gui, cli, server)
        customer_id: 고객 ID (로그 파일명에 포함)
    ...
    # 환경 변수에서 로그 파일 경로 확인 (자식 프로세스용)
    import os
    env_log_path = os.environ.get("SKINLENS_LOG_FILE")
    if env_log_path:
        _log_file_path = Path(env_log_path)
        # 자식 프로세스도 로깅 설정을 수행 (같은 로그 파일 사용)
        # 초기화 플래그는 설정하지 않음 (자식 프로세스도 독립적으로 로깅 설정 필요)
    ...
    if _log_file_path is None:
        ...
        # 모든 모드 공통: 날짜+시간별 로그 파일 (실행마다 고유한 파일)
        # 환경 변수로 로그 파일 경로를 전달하므로 자식 프로세스도 같은 파일 사용
        # 고객 ID가 있으면 파일명에 포함
        if customer_id:
            _log_file_path = log_dir / f"skinlens_{mode}_{customer_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        else:
            _log_file_path = log_dir / f"skinlens_{mode}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
    ...
    # 환경 변수에 로그 파일 경로 설정 (자식 프로세스용)
    import os
    os.environ["SKINLENS_LOG_FILE"] = str(log_file)
