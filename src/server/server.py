...
from src.utils.config import get_db_path_from_env, get_skin_analysis_db_path_from_env, load_config as _load_config
from src.utils.utils import _load_logging_level, setup_logging
...
# ── 라우터 임포트 ──────────────────────────────────────────────────────────
from src.server.routers import jobs, logs, stats, auth, customer, admin, websocket, health, orders, upload, integration, enhancements, app_features, images

# config.json에서 로그 레벨 로드 및 중앙 집중식 로깅 설정
...
async def lifespan(app: FastAPI):
    # startup
    # [구조변경] JWT_SECRET_KEY 기본값 체크 제거
    # 공유 secrets 모듈(src/common/secrets.py)이 프로덕션 fail-fast 로직을 담당
    ...
    # 자동 복구 엔진 초기화
    db = SkinAnalysisDB(db_path=get_skin_analysis_db_path_from_env())
