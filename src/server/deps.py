...
from src.utils.config import get_db_path_from_env, get_skin_analysis_db_path_from_env
...
# 테스트용 인증 우회
def create_test_token(username: str, role: str = "customer") -> str:
    """테스트용 JWT 토큰 생성 (테스트 환경 전용)"""
    if not TESTING_MODE:
        raise ValueError("테스트 환경에서만 사용 가능합니다")
    
    expires_delta = timedelta(hours=24)  # 테스트용으로 긴 유효기간
    to_encode = {
        "sub": username,
        "role": role,
        "exp": datetime.now(timezone.utc) + expires_delta
    }
    encoded_jwt = jwt.encode(to_encode, os.environ.get("JWT_SECRET_KEY", "test-secret"), algorithm="HS256")
    return encoded_jwt
...
def get_environment() -> str:
    """config.json의 environment 라벨 반환 (config reload 지원).

    환경변수(APP_ENV 등)가 별도로 설정돼 있으면 공유 secrets 모듈이 그것도 함께
    고려하므로, 여기서는 config 기준 라벨만 반환한다.
    """
    return _get_config().get("environment", "development")
...
_REFRESH_TOKEN_EXPIRE_DAYS = _jwt_config_cache.get("refresh_token_expire_days", 7)
...
def get_secret_key() -> str:
    ...
    [구조변경] 공유 secrets 모듈로 위임 → engine 과 동일 소스/정책 사용.
    프로덕션(config environment 또는 환경변수)인데 미설정이면 fail-fast.
    """
    from src.common.secrets import get_hmac_secret, is_production
    jwt_cfg = get_jwt_config()
    env_var = jwt_cfg.get("secret_key_env", "JWT_SECRET_KEY")
    prod = is_production(get_environment())
    return get_hmac_secret(env_var, production=prod)
...
def get_refresh_token_expire_days() -> int:
    """리프레시 토큰 만료 시간(일) 반환 (config reload 지원)."""
    return get_jwt_config().get("refresh_token_expire_days", 7)
...
REFRESH_TOKEN_EXPIRE_DAYS = _REFRESH_TOKEN_EXPIRE_DAYS  # [DEPRECATED] get_refresh_token_expire_days() 사용 권장
...
MAX_WORKERS = get_max_workers()  # [DEPRECATED] get_max_workers() 사용 권장
...
# ── SkinAnalysisDB Singleton ───────────────────────────────────────────────────

_skin_db_instance: Optional[SkinAnalysisDB] = None
_skin_db_lock = threading.Lock()


def get_skin_db() -> SkinAnalysisDB:
    """FastAPI Dependency for SkinAnalysisDB (싱글톤)."""
    global _skin_db_instance
    with _skin_db_lock:
        if _skin_db_instance is None:
            _skin_db_instance = SkinAnalysisDB(db_path=get_skin_analysis_db_path_from_env())
        return _skin_db_instance
