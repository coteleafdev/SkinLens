...
GET    /v1/customer/my/profile
PUT    /v1/customer/my/profile
DELETE /v1/customer/my/account
GET    /v1/customer/my/devices
POST   /v1/customer/my/devices
DELETE /v1/customer/my/devices/{device_id}
GET    /v1/customer/my/surveys
GET    /v1/customer/my/surveys/{survey_id}
PUT    /v1/customer/my/surveys/{survey_id}
DELETE /v1/customer/my/surveys/{survey_id}
GET    /v1/customer/my/analyses/{analysis_id}/download
"""
...
async def compare_analyses(
    ...
    try:
        ...
        # 새로운 구조: analysis_result 사용
        analysis_1_data = result_1.get("analysis_result", {})
        analysis_2_data = result_2.get("analysis_result", {})

        internal_1 = analysis_1_data
        internal_2 = analysis_2_data
...
async def update_profile(
    ...
    try:
        ...
        if profile_data.username and profile_data.username != user["username"]:
            # username 중복 확인 필요
            existing_user = db.get_user_by_username(profile_data.username)
            if existing_user:
                raise HTTPException(status_code=409, detail="Username already exists")
            
            # username 업데이트
            db.update_user_username(user["username"], profile_data.username)
        
        _audit(db, customer_id, ep, "PUT", current_customer.get("role"), request, ok=True)
        return {"message": "Profile updated successfully"}
    except (sqlite3.Error, ValueError) as e:
        log.error("프로필 수정 실패: %s", e)
...
async def delete_account(
    ...
    try:
        ...
        _audit(db, customer_id, ep, "DELETE", current_customer.get("role"), request, ok=True)
        return {"message": "Account deleted successfully"}
    except (sqlite3.Error, ValueError) as e:
        log.error("계정 삭제 실패: %s", e)
...
async def register_device(
    ...
    try:
        ...
        _audit(db, customer_id, ep, "POST", current_customer.get("role"), request, ok=True)
        return {"message": "Device registered successfully"}
    except (sqlite3.Error, ValueError) as e:
        log.error("장치 등록 실패: %s", e)
...
async def revoke_device(
    ...
    """장치 폐기."""
    ep = f"/v1/customer/my/devices/{device_id}"
    if current_customer is None:
        raise HTTPException(status_code=401, detail="Unauthorized")
    customer_id = current_customer.get("sub")
...
async def get_surveys(
    ...
    try:
        ...
        log.error("설문 목록 조회 실패: %s", e)
        raise HTTPException(status_code=500, detail="Failed to retrieve surveys")


@router.get("/surveys/{survey_id}", response_model=None)
async def get_survey(
    ...
    """설문 데이터 조회."""
    ep = f"/v1/customer/my/surveys/{survey_id}"
    if current_customer is None:
        raise HTTPException(status_code=401, detail="Unauthorized")
    customer_id = current_customer.get("sub")
    ...
    try:
        ...
        log.error("설문 조회 실패: %s", e)
        raise HTTPException(status_code=500, detail="Failed to retrieve survey")


@router.put("/surveys/{survey_id}", response_model=None)
async def update_survey(
    survey_id: str,
    survey_data: SurveyUpdateRequest,
    current_customer: Dict[str, Any] = Depends(get_current_customer),
    request: Request = None,
):
    """설문 데이터 수정."""
    ...
    try:
        ...
        success = db.update_survey(survey_id, survey_data.survey_data)
        
        if not success:
            raise HTTPException(status_code=400, detail="Failed to update survey")
        ...
        log.error("설문 수정 실패: %s", e)
        raise HTTPException(status_code=500, detail="Failed to update survey")


@router.delete("/surveys/{survey_id}", response_model=None)
async def delete_survey(
    ...
    try:
        ...
        success = db.delete_survey(survey_id)
        
        if not success:
            raise HTTPException(status_code=400, detail="Failed to delete survey")
        ...
        log.error("설문 삭제 실패: %s", e)
        raise HTTPException(status_code=500, detail="Failed to delete survey")


@router.get("/analyses/{analysis_id}/download", response_model=None)
async def download_analysis_image(
    analysis_id: int,
    image_type: str = "restored",
    current_customer: Dict[str, Any] = Depends(get_current_customer),
    request: Request = None,
):
    """분석 결과 이미지 다운로드."""
    ep = f"/v1/customer/my/analyses/{analysis_id}/download"
    if current_customer is None:
        raise HTTPException(status_code=401, detail="Unauthorized")
    customer_id = current_customer.get("sub")
    ...
    try:
        ...
        # 이미지 경로 결정
        json_result = analysis.get("json_result", {})
        # 새로운 구조: input_image, restored_image 사용
        if image_type == "restored":
            image_path = json_result.get("restored_image")
        elif image_type == "input":
            image_path = json_result.get("input_image")
        else:
        ...
        return FileResponse(
            ...
            filename=f"analysis_{analysis_id}_{image_type}.png"
        )
    except HTTPException:
        raise
