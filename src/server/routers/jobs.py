...
async def _run_job(job_id: str) -> None:
    ...
    웹서버는 엔진 서버에 HTTP 요청을 보내 분석을 위임하고,
    엔진 서버로부터 결과를 수신하여 저장합니다.
    """
    ...
    try:
        ...
        # 진행률 보고: 작업 시작
        await report_progress(job_id, "init", 0, "엔진 서버로 분석 위임 중...")

        # 엔진 서버 URL
        engine_url = os.environ.get("ENGINE_SERVER_URL", "http://localhost:8001")
        
        # 진행률 보고: 엔진 서버 통신
        await report_progress(job_id, "delegating", 10, "엔진 서버와 통신 중...")
        
        # 이미지 파일 읽기
        image_path = Path(meta["input_image_path"])
        if not image_path.exists():
            raise FileNotFoundError(f"Image not found: {image_path}")
        
        # 분석 요청 데이터 준비
        # [구조변경] lateral_images(서버 로컬 경로)는 더 이상 JSON 으로 전달하지 않는다.
        # 측면 이미지는 아래에서 multipart(images[]+angles[])로 실제 전송하고,
        # engine 이 자신의 로컬 경로로 lateral_images 를 재구성한다.
        analysis_request = {
        ...
        import json
        import hmac
        payload_str = json.dumps(analysis_request, sort_keys=True)
        signature = hmac.new(
        ...
        async with httpx.AsyncClient(timeout=300) as client:
            # 엔진 서버로 요청 전송
            # [구조변경] front 만이 아니라 모든 각도 이미지를 multipart 로 전송한다.
            # images[] (파일) + angles[] (폼 필드) + analysis_request/signature (폼 필드)
            lateral = meta.get("lateral_images") or [
                {"angle": "front", "path": str(image_path)}
            ]
            _open_handles = []
            files = []
            data = []
            try:
                for item in lateral:
                    ...
                    data.append(("angles", item.get("angle", "front")))
                data.append(("analysis_request", payload_str))
                data.append(("signature", f"sha256={signature}"))

                response = await client.post(
                    f"{engine_url}/v1/engine/analysis/jobs",
                    files=files,
                    data=data,
                )
            ...
            meta["engine_job_id"] = engine_job_id
            write_job_meta(job_id, meta)
        
            # 진행률 보고: 엔진 서버 처리 중
            await report_progress(job_id, "processing", 30, "엔진 서버에서 분석 처리 중...")
            ...
            while elapsed < max_wait:
                ...
                status_response = await client.get(
                    f"{engine_url}/v1/engine/analysis/jobs/{engine_job_id}"
                )
                status_response.raise_for_status()
                status_data = status_response.json()
            
                # 진행률 업데이트
                progress = min(30 + int((elapsed / max_wait) * 50), 80)
                await report_progress(job_id, "processing", progress, "엔진 서버 처리 중...")
                ...
                if status_data["status"] == "succeeded":
                    ...
                    meta["artifacts_local"] = artifacts_local
                    meta["result"] = result
                    write_job_meta(job_id, meta)
                    ...
                    log.info(f"[Job {job_id}] 엔진 서버 분석 완료: engine_job_id={engine_job_id}")
                    break
                    ...
                    meta["status"] = "failed"
                    meta["finished_at"] = _utc_now_iso()
                    meta["error"] = f"Engine server failed: {error}"
                    write_job_meta(job_id, meta)
                
                    # 진행률 보고: 에러
                    from src.server.routers.websocket import manager
                    await manager.send_error(job_id, meta["error"])
                
                    log.error(f"[Job {job_id}] 엔진 서버 분석 실패: {error}")
                    break
        
        if meta["status"] == "running":
            meta["status"] = "failed"
            meta["finished_at"] = _utc_now_iso()
            meta["error"] = "Engine server timeout"
            write_job_meta(job_id, meta)
            
            from src.server.routers.websocket import manager
            await manager.send_error(job_id, "Engine server timeout")
        ...
        if callback_url and meta["status"] == "succeeded":
            ...
            except (httpx.HTTPError, ValueError) as callback_err:
                log.warning("콜백 URL 호출 실패: %s", callback_err)

    except (OSError, IOError, RuntimeError, ValueError, KeyError, TypeError, httpx.HTTPError) as e:
