...
5. LLM AI 소견 작성 (llm_prompt_template.md 사용)
6. 결과 JSON 출력
...
# 중앙 집중식 로깅 설정 (초기 설정, main()에서 customer_id로 재설정)
...
def _update_log_file_with_customer_id(customer_id: str) -> None:
    ...
    if file_handler:
        ...
        new_path = log_dir / f"skinlens_cli_{customer_id}_{timestamp}.log"
    ...
    *,
    ...
    if use_multi_view_analysis and left45_path and right45_path:
        ...
        with ThreadPoolExecutor(max_workers=3) as executor:
            ...
            # [FIX] 다중뷰 분기에서도 정면(front) 결과를 대표 restore_result로 할당
            # → 하위 결과 통합부(restoration_stats / pipeline_status)의 NameError 방지
            restore_result: PipelineResult = results[0]
        ...
        try:
            ...
            if do_restore:
                ...
                llm_result = reporter.generate_report_from_dual_images(
                    ...
                    ref_image_path=str(restored_image),
                    ref_measurements_report=rest_analysis.get("measurements_report", {}),
                    ref_overall_score=float(rest_analysis.get("overall_score", 0)),
                    ref_perceived_age=float(rest_analysis.get("perceived_age", 0)),
                    provide_scores=llm_scores,  # [FIX] GUI와 동일하게 llm_scores 사용 (기본 False)
            ...
            if isinstance(llm_result, tuple):
                # 듀얼 이미지 모드: (orig_report, ref_report)
                orig_report, ref_report = llm_result
                analysis_result["llm_report"] = {
                    "original": report_to_dict(orig_report),
                    "reference": report_to_dict(ref_report),
                }
            ...
            log.info("LLM AI 소견 작성 완료")
        except Exception as e:
            log.warning("LLM AI 소견 작성 실패: %s", e)
            analysis_result["llm_report"] = None
    
    # ── 처방전 기록 ─────────────────────────────────────────────────────
    # 제조사에서 맞춤형 화장품 제조를 위해 처방전을 별도로 기록
    try:
        from src.prescription.prescription_calculator import create_prescription
        
        full_prescription = create_prescription(
            skin_assessment_scores=analysis_result.get("measurements_report", {}),
        ...
        # [REFACTOR 2026-06-08] create_prescription가 믹스 이름을 직접 포함하므로
        # 별도 enrich/config 조회가 불필요.
        analysis_result["prescription"] = full_prescription
        log.info("처방전 기록 완료 (믹스 코드 이름 포함)")
    except Exception as e:
        log.warning("처방전 기록 실패: %s", e)
        analysis_result["prescription"] = None
    ...
    settings_info = {
        ...
        "analyzer_score_tune": get_cli_defaults().get("analyzer_score_tune", True),
        "device": _get_config().get("device", {}).get("restoreformer_device", "auto")
    }
    ...
    # 7. 버전 정보
    version_info = {
        "config_version": _get_config().get("version", "unknown"),
    ...
    # resource_stats는 나중에 초기화되므로 여기서는 건너뜀 (아래에서 처리)
    ...
    # resource_stats가 있으면 environment_info에 추가
    if resource_stats:
...
async def run_analysis_pipeline_async(
    ...
    *,
    ...
    try:
        # 동기 함수를 비동기로 실행
        # [FIX P0] run_analysis_pipeline 은 output_dir 이후가 전부 keyword-only.
        # run_in_executor 는 위치 인자만 전달하므로 functools.partial 로 키워드를 묶는다.
        # (기존 위치 전달은 TypeError 및 인자 어긋남 발생)
...
async def main_async(args) -> None:
    ...
    try:
        from src.pipeline.analysis_service import AnalysisService  # [#3] 단일 진입점 (지연 import)
...
예시:
  ...
  python skin_analysis_cli.py -i input.jpg -o output_dir --async
    ...
    parser.add_argument(
        "-o", "--output",
        type=Path,
        default=Path(_get_config().get("paths", {}).get("output_dir", "results")),
    ...
    parser.add_argument(
        "--include-base64",
        action="store_true",
        default=get_cli_defaults().get("include_base64", False),
    ...
    parser.add_argument(
        ...
        default=get_cli_defaults().get("score_safety_net", True),
    ...
    parser.add_argument(
        "--analyzer-score-tune",
        action=argparse.BooleanOptionalAction,
        default=get_cli_defaults().get("analyzer_score_tune", True),
        help=(
    ...
    parser.add_argument(
        "--llm-report",
        action="store_true",
        default=get_cli_defaults().get("llm_report", True),
        help="LLM AI 소견 작성 (기본 끔)"
    )

    parser.add_argument(
        "--llm-scores",
        action=argparse.BooleanOptionalAction,
        default=get_cli_defaults().get("llm_scores", False),
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="LLM API 키 (지정하지 않으면 환경 변수 GOOGLE_API_KEY 사용)"
    )
    ...
    parser.add_argument(
        "--use-multi-view",
        action=argparse.BooleanOptionalAction,
        default=get_cli_defaults().get("use_multi_view_analysis", True),
    ...
    customer_id = getattr(args, 'customer_id', None)
    if not customer_id:
        # 입력 파일명에서 customer_id 추출
        if args.input:
            customer_id = args.input.stem
    
    if customer_id:
        _update_log_file_with_customer_id(customer_id)
