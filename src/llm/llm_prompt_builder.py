...
def _build_reference_guided_prompt(
    ...
    Args:
        ...
        ref_measurements_report:   복원 CV 측정값 (복원본 분석 품질 참고용)
    ...
    # ── 4. 포맷팅 ─────────────────────────────────────────────────
    # CV 점수 섹션 생성 (provide_scores=True일 때만)
    cv_scores_section = ""
    if provide_scores:
        cv_scores_section = _build_cv_scores_section(
    ...
    format_dict = {
        ...
        "ref_overall_score":   f"{ref_overall_score:.1f}",
        "ref_perceived_age":   f"{ref_perceived_age:.1f}",
        "score_criteria_section": score_criteria_section,
        "cv_scores_section":   cv_scores_section,
        "product_info":        product_info or "제공된 맞춤형 화장품 정보가 없습니다.",
    ...
    log.info(
        "[RGP] reference_guided 프롬프트 구성: provide_scores=%s, orig=%.1f점, ref=%.1f점",
        provide_scores, orig_overall_score, ref_overall_score,
    )
...
def _build_cv_scores_section(
    ...
    lines = [
        ...
        f"복원 종합: {ref_overall:.1f}점 / 인지 나이: {ref_age:.1f}세",
        "",
        "| 항목 | 원본 CV점수 | 복원 CV점수 |",
        "|------|------------|------------|",
    ]
    for key, display, _, _ in _METRIC_META:
        ...
        r = ref_report.get(key)
        o_str = f"{float(o):.1f}" if o is not None else "N/A"
        r_str = f"{float(r):.1f}" if r is not None else "N/A"
        lines.append(f"| {display} ({key}) | {o_str} | {r_str} |")
    lines.append("")
    lines.append(
...
def _build_dual_image_prompt(
    ...
    if not template:
        ...
        return build_fallback_dual_image_prompt(
            ...
            ref_measurements_report, ref_overall_score, ref_perceived_age,
            provide_scores
        )
    ...
    if not provide_scores:
        ...
        if user_prompt_template:
            ...
            log.info("[프롬프트 구성] 복원 종합 점수: %.1f, 인지 나이: %.1f세", ref_overall_score, ref_perceived_age)
    ...
    log.info("[프롬프트 구성] 복원 종합 점수: %.1f, 인지 나이: %.1f세", 
             ref_overall_score, ref_perceived_age)
