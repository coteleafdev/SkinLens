...
[CANONICAL] 본 모듈의 `LLMProvider` + `create_provider()` 가 SkinLens 의 **표준 LLM 추상화**입니다.
LlmSkinReporter 를 포함한 모든 활성 경로가 이 추상화를 사용합니다.
(병렬 추상화였던 src.llm.base.BaseLLM / src.llm.registry.LLMRegistry / src.llm.strategies 는
 외부 소비자가 없어 deprecated — 신규 코드는 본 모듈을 사용하세요.)
...
class LLMProvider(ABC):
    ...
    def generate_content(
        ...
        Args:
            ...
            temperature: 온도 파라미터 (None이면 기본값 사용)
...
class GeminiProvider(LLMProvider):
    ...
    def generate_content(
        ...
        # temperature가 제공되면 사용, 아니면 기본값 사용
