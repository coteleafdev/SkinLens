...
class SkinMeasurementCompareDialog(QDialog):
    ...
    def __init__(
        ...
        llm_ref_report: Optional[Any] = None,  # 복원 LLM 보고서
        ...
        self._last_llm_report_ref = llm_ref_report

        mo = orig_result.get("measurements_report") or orig_result.get("measurements", {})
        mi = ref_result.get("measurements_report") or ref_result.get("measurements", {})
        o_age = orig_result.get("perceived_age", "—")
        i_age = ref_result.get("perceived_age", "—")
        ...
        riv = _numeric_value(
            ref_result.get("overall_score_report") or ref_result.get("overall_score")
        )
        ...
        for cat_name, keys in get_measurement_categories():
            for key in keys:
                ...
                if mi.get(key) is not None:
                    item_ref = QTableWidgetItem(f"{int(round(vi))}")
                else:
                    item_ref = QTableWidgetItem("N/A")
                    item_ref.setBackground(QColor(240, 240, 240))
                ...
                # 복원 LLM 점수 (초기에는 빈 값)
                item_llm_ref = QTableWidgetItem("")
                item_llm_ref.setBackground(QColor(240, 240, 240))
                self.table.setItem(row, 4, item_llm_ref)
        ...
        # LLM 결과가 이미 있으면 표시
        if self._last_llm_report_orig and self._last_llm_report_ref:
            self._on_llm_dual_finished(self._last_llm_report_orig, self._last_llm_report_ref, 0.0)
    ...
    def _generate_llm_report_dual(self) -> None:
        ...
        ref_measurements_report = self._ref_result.get("measurements_report") or self._ref_result.get("measurements", {})
        ref_overall_score = self._ref_result.get("overall_score", 0)
        ref_perceived_age = self._ref_result.get("perceived_age", 0)
    ...
    def _on_llm_dual_finished(self, orig_report: object, ref_report: object, elapsed_time: float = 0.0) -> None:
        """듀얼 이미지 LLM 분석 완료"""
        # 스레드가 있는 경우에만 종료 처리
        ...
        # 소견 텍스트 구성
        adjustment_note = ""
        if self._llm_scores and (getattr(orig_report, 'scores_adjusted', False) or getattr(ref_report, 'scores_adjusted', False)):
        ...
        # recommendation이 리스트인 경우 상위 3개만 표시
        if isinstance(recommendation_text, list):
            recommendation_text = '\n'.join(recommendation_text[:3])
        ...
        # 복원 LLM 측정 여부 확인
        ref_has_llm = (
            hasattr(ref_report, 'metric_opinions') and 
            len(ref_report.metric_opinions) > 0
        )
        
        # 복원 LLM 점수 표시
        if ref_has_llm:
            ref_llm_score_text = f"- 기준 LLM 측정 피부건강지수: {int(round(ref_report.overall_score))}점"
        else:
            ref_llm_score_text = "- 기준 LLM 측정 피부건강지수: -"
...
- 기준 인지나이: {int(round(ref_report.perceived_age))}세
"""
        ...
        try:
            ...
            full_prescription = create_prescription(
                skin_assessment_scores=self._orig_result.get("measurements_report", {}),
                pcr_result=None,  # PCR 데이터가 없으면 None
                age_group_statistics=None,
                age=self._orig_result.get("perceived_age", 30) or 30,
                gender="female",  # 기본값
                skin_type=None,  # 향후 설문 연동 시
                concerns=None,  # 향후 설문 연동 시
            )
            
            # [REFACTOR 2026-06-08] 믹스 이름은 처방전 객체에 직접 포함(SSOT). config 조회 불필요.
            
            # 처방전 텍스트 구성 (상세 테이블에 표시되므로 여기서는 비워둠)
            prescription_text = ""
            
            # 처방전을 LLM 소견에 추가
            updated_report_text = report_text + prescription_text
            self.llm_report_text.setText(updated_report_text)
            
            # 처방전 저장 (엑셀/HTML 내보내기용)
            self._last_prescription = full_prescription
            log.info("[GUI] 처방전 LLM 소견에 추가 완료")
        except Exception as e:
            log.warning(f"[GUI] 처방전 추가 실패: {e}")
            self._last_prescription = None

        # Report 저장 (엑셀 내보내기 시 항목별 소견 출력용)
        self._last_llm_report_orig = orig_report
        self._last_llm_report_ref = ref_report
        ...
        self._update_table_with_llm_scores(orig_report.metric_opinions, ref_report.metric_opinions)
        ...
        if ref_has_llm:
            self._update_table_with_llm_overall_both(orig_report.overall_score, ref_report.overall_score)
        else:
    ...
    def _update_table_with_llm_overall_both(self, orig_overall_score: float, ref_overall_score: float) -> None:
        """원본/기준 이미지의 LLM 피부건강지수를 라벨에 표시합니다.

        Args:
            orig_overall_score: 원본 이미지 LLM 피부건강지수
            ref_overall_score: 기준 이미지 LLM 피부건강지수
        """
        self.llm_overall_label.setText(f"LLM 피부건강지수 - 원본: {int(round(orig_overall_score))}점 / 기준: {int(round(ref_overall_score))}점")

        # 테이블에는 피부건강지수를 표시하지 않음 (항목별 점수만 표시)
        # 피부건강지수는 라벨에만 표시하여 항목별 점수와 혼동 방지
    
    @staticmethod
    def _sanitize_cell_text(text: str | List[Any] | Any) -> str:
    ...
    def _export_to_excel(self) -> None:
        ...
        try:
            ...
            try:
                ...
                # 기준 이미지
                img_ref = OpenpyxlImage(str(self._ref_path))
                img_ref.width = 200
                img_ref.height = 200
                img_ref.anchor = "D1"
                ws.add_image(img_ref)
            ...
            ref_overall = _numeric_value(
                self._ref_result.get("overall_score_report")
                or self._ref_result.get("overall_score")
            )

            # LLM 피부건강지수는 LLM 리포트의 overall_score 사용
            orig_llm_overall: Optional[float] = None
            ref_llm_overall: Optional[float] = None
            ...
            ref_has_llm = (
                self._last_llm_report_ref and 
                hasattr(self._last_llm_report_ref, 'metric_opinions') and 
                len(self._last_llm_report_ref.metric_opinions) > 0
            )
            if ref_has_llm and hasattr(self._last_llm_report_ref, 'overall_score'):
                ref_llm_overall = _numeric_value(self._last_llm_report_ref.overall_score)

            # ── 메타 헤더를 전부 append_with_font_local() 로 통일 ──────

            # 메타 정보 행 추가 (고객용 보고서: AI 측정 점수만 표시)
            append_with_font_local(["분석 이미지", str(self._orig_path)], bold_font)
            if orig_llm_overall is not None:
                append_with_font_local(["AI 피부건강지수 (원본)", f"{int(round(orig_llm_overall))}"], bold_font)
            if ref_has_llm and ref_llm_overall is not None:
                append_with_font_local(["AI 피부건강지수 (기준)", f"{int(round(ref_llm_overall))}"], bold_font)
            # 엄격한 평가 모드 표시
            try:
                from src.utils.config import load_config as _load_config
                config = _load_config()
            ...
            # ── 점수등급 기준 ─────────────────────────────────────────
            append_with_font_local(["점수등급 기준"], bold_font)
            append_with_font_local(["점수 범위", "등급"], bold_font)
            append_with_font_local(["90 이상", "매우 우수"])
            append_with_font_local(["80~90", "우수"])
            append_with_font_local(["70~80", "양호"])
            append_with_font_local(["60~70", "집중케어 추천"])
            append_with_font_local(["60 미만", "개선 필요"])
            append_with_font_local([])  # 빈 행

            # ── 테이블 데이터 추가 (고객용 보고서: AI 측정 점수만 표시) ─────────────────────────────────
            # 헤더 (항목명, AI 측정 점수 원본, AI 측정 점수 기준)
            customer_headers = ["항목명", "AI 측정 점수 (원본)", "AI 측정 점수 (기준)"]
            append_with_font_local(customer_headers, bold_font)
            ...
            for r in range(self.table.rowCount()):
                ...
                # 원본 AI 점수 (col 3)
                item_llm_orig = self.table.item(r, 3)
                llm_orig_text = item_llm_orig.text() if item_llm_orig else ""
                
                # 기준 AI 점수 (col 4)
                item_llm_ref = self.table.item(r, 4)
                llm_ref_text = item_llm_ref.text() if item_llm_ref else ""
                
                append_with_font_local([name_text, llm_orig_text, llm_ref_text])

            # ── AI 소견 추가 ─────────────────────────────────────
            append_with_font_local([])  # 빈 행
            append_with_font_local(["AI 소견"], bold_font)
            append_with_font_local([])  # 빈 행
            ...
            if self._last_llm_report_orig:
                ...
                if matched_products and isinstance(matched_products, list):
                    ...
                    for i, product in enumerate(matched_products, 1):
                        ...
                        append_with_font_local([f"매칭 점수: {int(round(match_score_float * 100))}%"], small_font)
                        append_with_font_local([f"주요 성분: {self._sanitize_cell_text(key_ingredients)}"], small_font)
                        append_with_font_local([f"효능: {efficacy}"], small_font)
                        append_with_font_local([f"추천 이유: {match_reason}"], small_font)
                        append_with_font_local([])  # 빈 행

            # 처방전 정보 추가 (상세 테이블에 표시되므로 여기서는 비워둠)
            # 처방전 섹션은 제거됨
            ...
            try:
                from src.utils.config import load_config as _load_config
                config = _load_config()
                active_mixes_config = config.get("prescription", {}).get("mix_codes", {})
                ...
                if active_mixes_config:
                    ...
                    for mix_code in sorted(active_mixes_config.keys()):
                        if mix_code.startswith("M") and mix_code != "_note":
                            mix_info = active_mixes_config[mix_code]
                            ...
                            # 처방전에서 배합비 추출
                            percentage = ""
                            if hasattr(self, '_last_prescription') and self._last_prescription:
                                # assessment에서 배합비 추출
                                assessment = self._last_prescription.get("assessment", {})
                                ...
                                if mix_code == "M01" and "base" in self._last_prescription:
                                    base_data = self._last_prescription.get("base", {})
                                    if isinstance(base_data, dict):
                                        percentage = f"{base_data.get('percentage', 0)}%"
                                    else:
                                        percentage = f"{base_data}%"
                            
                            append_with_font_local([mix_code, name, category, description, ingredients, percentage], small_font)
                    
                    append_with_font_local([])  # 빈 행
            except Exception as mix_err:
                log.warning(f"활성 믹스 테이블 추가 실패: {mix_err}")

            # ── PCR 믹스 테이블 추가 ─────────────────────────────────────────
            try:
                from src.utils.config import load_config as _load_config
                config = _load_config()
                pcr_mixes_config = config.get("prescription", {}).get("mix_codes", {})
                
                if pcr_mixes_config:
                    append_with_font_local([])  # 빈 행
                    append_with_font_local(["PCR 믹스 (PM01-PM07)"], bold_font)
                    append_with_font_local(["코드", "이름", "카테고리", "설명", "주요 성분", "배합비"], bold_font)
                    
                    for mix_code in sorted(pcr_mixes_config.keys()):
                        if mix_code.startswith("PM") and mix_code != "_note":
                            mix_info = pcr_mixes_config[mix_code]
                            name = mix_info.get("name", "")
                            category = mix_info.get("category", "")
                            description = mix_info.get("description", "")
                            ingredients = ", ".join(mix_info.get("ingredients", []))
                            ...
                            percentage = ""
                            if hasattr(self, '_last_prescription') and self._last_prescription:
                                pcr_prescription = self._last_prescription.get("pcr", {})
                ...
                log.warning(f"PCR 믹스 테이블 추가 실패: {mix_err}")
    ...
    def _export_to_html(self) -> None:
        ...
        try:
            ...
            if orig_image_path and Path(orig_image_path).exists():
                try:
                    orig_image_uri = image_to_base64(orig_image_path)
                except Exception as e:
                    log.warning(f"원본 이미지 변환 실패: {e}")
            
            if ref_image_path and Path(ref_image_path).exists():
                try:
                    ref_image_uri = image_to_base64(ref_image_path)
                except Exception as e:
                    log.warning(f"복원 이미지 변환 실패: {e}")
            
            # 테이블 데이터 추출 (고객용 보고서: 항목명, AI 측정 점수 원본, AI 측정 점수 기준)
            table_data = []
            for row in range(self.table.rowCount()):
                ...
                table_data.append([name_text, llm_orig_text, llm_ref_text])
            ...
            matched_products = []
            metric_opinions = {}
            orig_llm_overall_score = None
            ref_llm_overall_score = None
            
            if self._last_llm_report_orig:
                llm_overall_opinion = self._last_llm_report_orig.overall_opinion or ""
                llm_recommendations = self._last_llm_report_orig.recommendation or []
                if isinstance(llm_recommendations, str):
                    llm_recommendations = [llm_recommendations]
                matched_products = self._last_llm_report_orig.matched_products or []
                
                # 원본 LLM 피부건강지수 추출
                if hasattr(self._last_llm_report_orig, 'overall_score'):
                    orig_llm_overall_score = _numeric_value(self._last_llm_report_orig.overall_score)
                
                # 항목별 소견 추출
                if hasattr(self._last_llm_report_orig, 'metric_opinions'):
                    # metric_opinions는 List[MetricOpinion] 형태
                    for metric_opinion in self._last_llm_report_orig.metric_opinions:
                        if hasattr(metric_opinion, 'display_name') and hasattr(metric_opinion, 'opinion'):
                            metric_opinions[metric_opinion.display_name] = metric_opinion.opinion
            
            # 기준 LLM 피부건강지수 추출
            if self._last_llm_report_ref and hasattr(self._last_llm_report_ref, 'overall_score'):
                # 복원 LLM 측정 여부 확인
                ref_has_llm = (
                    hasattr(self._last_llm_report_ref, 'metric_opinions') and 
                    len(self._last_llm_report_ref.metric_opinions) > 0
                ...
                if ref_has_llm:
                    ref_llm_overall_score = _numeric_value(self._last_llm_report_ref.overall_score)
            ...
            try:
                ...
                # PCR 믹스 (PM01-PM07)
                pcr_mixes = {k: v for k, v in mixes_config.items() if k.startswith("PM") and k != "_note"}
                
                log.info(f"active_mixes loaded: {len(active_mixes)} items")
                log.info(f"pcr_mixes loaded: {len(pcr_mixes)} items")
            except Exception as mix_err:
                log.warning(f"믹스 정보 로드 실패: {mix_err}")
            
            # HTML 생성 (고객용 보고서: 원본/기준 이미지 표시, AI 점수는 원본/기준 모두 표시)
            html_content = generate_html_report(
                title="피부 분석 보고서",
                original_image_uri=orig_image_uri,
                restored_image_uri=ref_image_uri,  # 기준 이미지 표시
                ...
                orig_llm_overall_score=orig_llm_overall_score,
                ref_llm_overall_score=ref_llm_overall_score,  # 기준 AI 점수 표시
            ...
            QMessageBox.information(self, "완료", f"HTML 파일 저장 완료:\n{file_path}")
            log.info("HTML 내보내기 완료: %s", file_path)
            
        except Exception as e:
            error_detail = traceback.format_exc()
            log.error("=" * 60)
            log.error("HTML 내보내기 실패")
            log.error("오류 타입: %s", type(e).__name__)
            log.error("오류 메시지: %s", str(e))
            log.error("상세 스택 트레이스:")
            log.error("%s", error_detail)
