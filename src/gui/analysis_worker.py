...
class _AnalyzeWorker(QObject):
    """듀얼 이미지 분석 백그라운드 작업자"""
    finished = Signal(object, object)  # orig_result, ref_result
    failed = Signal(str)
    ...
    def run(self) -> None:
        ...
        try:
            ...
            ref_in, tmp_i = _prepare_analysis_path(
                Path(self._ref_path), "ref", self._max_side, t0=t0,
            )
            ...
            _dlog(f"analyze_all(보정) 시작 path={ref_in!r} ref_stat=없음(기준 생성)", t0=t0)
            ref = an.analyze_all(ref_in, debug=False, clahe_preprocessed=False)
            _dlog(
                f"analyze_all(보정) 완료 overall={ref.get('overall_score')!r}",
                t0=t0,
            )

            ref_stat = ref.get("skin_stat") if self._use_ref_stat else None
            ...
            _dlog(
                ...
                f"ref_stat={'있음' if ref_stat is not None else '없음(비활성 또는 없음)'}",
                t0=t0,
            )
            try:
                orig = an.analyze_all(
