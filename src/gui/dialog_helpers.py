...
def show_skin_measurement_compare_dialog(
    ...
    if modal:
        ...
        try:
            ...
            log.info("피부 비교 분석 시작 (동기): orig=%s ref=%s", orig_path, ref_path)
            analysis_start = time.time()
            result_orig = analyzer.analyze_all(str(orig_path.resolve()))
            result_ref = analyzer.analyze_all(str(ref_path.resolve()))
            ...
            log.debug(f"modal=True: result_ref keys={list(result_ref.keys())}")
            ...
            if llm_json_path and llm_json_path.exists():
                ...
                staged_files = list(ref_path.parent.glob("00_input_*.png"))
                if staged_files:
                    input_filename = staged_files[0].stem  # 확장자 제거 (예: 00_input_색소침착_트러블_홍조)
                else:
                    input_filename = orig_path.stem  # 원본 파일명
                results_json_path = ref_path.parent / f"{input_filename}.json"
            ...
            if results_json_path.exists():
                try:
                    ...
                    # 새로운 구조: analysis_result.llm_report
                    analysis_result = results_data.get("analysis_result", {})
                    llm_analysis = analysis_result.get("llm_report", {})
                    if "original" in llm_analysis and "reference" in llm_analysis:
                        log.info(f"[LLM JSON] modal=True: JSON에서 LLM 결과 읽음 (analysis_result.llm_report 키 존재)")
                        # LLM 결과를 SkinLLMReport 객체로 변환
                        from src.llm.llm_formatters import SkinLLMReport, MetricOpinion

                        orig_data = llm_analysis["original"]
                        llm_orig_result = SkinLLMReport(
                        ...
                        ref_data = llm_analysis["reference"]
                        llm_ref_result = SkinLLMReport(
                            overall_opinion=ref_data.get("overall_opinion", ""),
                            overall_score=ref_data.get("overall_score", 0),
                            perceived_age=ref_data.get("perceived_age", 0),
                            metric_opinions=[
                                MetricOpinion(
                                    key=m.get("key", ""),
                                    display_name=m["display_name"],
                                ...
                                for m in ref_data.get("metric_opinions", [])
                            ],
                        )
                        log.debug(f"modal=True: LLM 결과 로드 완료 ({results_json_path.name})")
                except Exception as e:
            ...
            # 안전장치 적용 여부 확인
            safety_net_applied = result_ref.get("safety_net_adjusted", False)
            ...
            if llm_orig_result is not None and llm_ref_result is not None:
                if (not llm_orig_result.overall_opinion and not llm_orig_result.metric_opinions and
                    not llm_ref_result.overall_opinion and not llm_ref_result.metric_opinions):
                    is_empty_result = True
                    log.debug(f"modal=True: JSON에서 빈 LLM 결과 읽음 (overall_opinion 빈 문자열, metric_opinions 빈 리스트)")
            if llm_json_path and llm_json_path.exists() and llm_orig_result is not None and llm_ref_result is not None and not is_empty_result:
    ...
    try:
        ...
        if not ref_path.is_file():
            if parent:
                QMessageBox.warning(parent, "분석", f"보정(기준) 이미지가 없습니다:\n{ref_path}")
            else:
                log.error("보정(기준) 이미지가 없습니다: %s", ref_path)
            return
        log.debug("파일 확인 완료: orig=%s, ref=%s", orig_path.name, ref_path.name)
        ...
        log.info(
            "피부 비교 분석 시작 orig=%s ref=%s debug_env=%s",
            orig_path,
            ref_path,
            _env_debug_enabled(),
        )
        ...
        def _finish_on_gui_thread_success(o: object, i: object) -> None:
            ...
            QTimer.singleShot(0, lambda: _show_compare_dialog(parent, orig_path, ref_path, o, i))

        def _show_compare_dialog(parent: Optional, orig_path: Path, ref_path: Path, o: object, i: object) -> None:
            """비교 다이얼로그를 비동기로 표시."""
            try:
                ...
                if llm_json_path and llm_json_path.exists():
                    try:
                        ...
                        llm_analysis = analysis_result.get("llm_report", {})
                        if "original" in llm_analysis and "reference" in llm_analysis:
                            log.info(f"[LLM JSON] modal=False: JSON에서 LLM 결과 읽음 (analysis_result.llm_report 키 존재)")
                            from src.llm.llm_formatters import SkinLLMReport, MetricOpinion
                            orig_data = llm_analysis["original"]
                            llm_orig_result = SkinLLMReport(
                                overall_opinion=orig_data.get("overall_opinion", ""),
                ...
                if llm_orig_result is not None and llm_ref_result is not None and not is_empty_result:
                    llm_provide_scores = False
