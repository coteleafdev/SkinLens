...
[BUGFIX 2026-06-07] NameError 및 UnicodeEncodeError 수정
  - NameError: cannot access local variable 'json' 수정 (execution_history.py, i18n.py)
  - UnicodeEncodeError: 'cp949' codec can't encode character '\u2014' 수정 (pipeline_core.py, supabase_sync.py)
  - 서버 로그 파일명 수정 (engine_server.py)
[STRUCTURAL 2026-06-07] 구조 변경 (engine 동시성 · 다중이미지 multipart · 공유 secrets)
  - 공유 secrets 모듈 추가 (src/common/secrets.py): server/engine 동일 시크릿 소스 사용
  - engine 동시성 세마포어 추가 (GPU 보호): ENGINE_MAX_CONCURRENCY 환경변수로 제어
  - 다중이미지 multipart 확장: engine에서 images[] + angles[] 수신, server에서 모든 각도 전송
...
- **피부 분석**: 18개 측정항목 기반 피부 상태 분석 (색소, 홍조, 모공, 주름, 텍스처, 톤, 탄력, 피부 타입)
...
- **주문 관리**: 주문 생성, 결제, 배송, 재고 관리, 통계/분석, 알림, 바코드, 라벨 프린터
...
│   ├── cli/                    # CLI 모듈
│   │   ├── entry.py            # CLI 엔트리포인트
│   │   └── skin_analysis_cli.py  # CLI 특화 로직 (레거시)
│   ├── common/                 # 공유 모듈 (secrets 등)
│   ├── config/                 # 설정 모듈
│   ├── gui/                    # GUI 모듈
│   │   ├── entry.py            # GUI 엔트리포인트
│   │   ├── gui_wrapper.py      # GUI 특화 로직
│   │   └── image_enhancer.py   # GUI 레거시 엔트리포인트
│   ├── server/                 # FastAPI 서버
│   ├── telegram/               # 텔레그램 모듈
│   ├── skin/                   # 피부 분석 코어
│   ├── pipeline/               # 파이프라인
│   │   ├── pipeline_core.py    # 파이프라인 코어
│   │   └── pipeline_runner.py  # 파이프라인 실행기 (공통)
│   ├── scoring/                # 점수 계산
│   ├── llm/                    # LLM 모듈
│   ├── db/                     # 데이터베이스
│   ├── restoration/            # 복원 모듈
...
│   └── utils/                  # 공통 유틸리티
├── tests/                      # 단위 테스트
│   └── test_pipeline_runner.py # 파이프라인 실행기 테스트
├── config/                     # 설정 파일
│   └── config.json             # 메인 설정
...
새로운 구조에서는 명명 규칙이 통일되고 파이프라인 로직이 중앙화되었습니다:

- **CLI 모드**: `src/cli/entry.py` → `src/pipeline/pipeline_runner.py`
- **GUI 모드**: `src/gui/entry.py` → `src/gui/gui_wrapper.py` → `src/pipeline/pipeline_runner.py`
...
- [ARCHITECTURE.md](docs/guides/ARCHITECTURE.md): 아키텍처
- [SKIN_SCORING_GUIDE.md](docs/guides/SKIN_SCORING_GUIDE.md): 스코어링 가이드
- [ORDER_MANAGEMENT_GUIDE.md](docs/guides/ORDER_MANAGEMENT_GUIDE.md): 주문 관리 기능 가이드 (주문 생성, 결제, 배송, 재고, 통계, 알림, 바코드, 라벨)
...
### 시뮬레이션 가이드 (simulation/)
- [TESTING_AND_SIMULATION_GUIDE.md](simulation/TESTING_AND_SIMULATION_GUIDE.md): 전체 테스트 및 시뮬레이션 종합 가이드
- [DOCKER_SIMULATION_GUIDE.md](simulation/DOCKER_SIMULATION_GUIDE.md): Docker 시뮬레이션 가이드 (컨테이너 관리, 분석 테스트)
- [SERVER_TEST_GUIDE.md](simulation/SERVER_TEST_GUIDE.md): 서버 테스트 가이드 (API 테스트, 통합 테스트)
...
- `ENGINE_MAX_CONCURRENCY`: 동시 실행 한도 (기본: 2)
...
- **V1_WEIGHTS**: v1.0 피부건강지수 가중치를 config.json으로 이동 (10개 항목)
