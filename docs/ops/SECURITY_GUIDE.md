...
**API 엔드포인트:**
- POST `/v1/admin/api-keys` - API 키 생성
- GET `/v1/admin/api-keys` - API 키 목록 조회
- DELETE `/v1/admin/api-keys/{key_id}` - API 키 폐지
...
```bash
# API 키 생성
curl -X POST "http://localhost:8000/v1/admin/api-keys?name=TestKey&owner_id=CUST001&scopes=%5B%22read%22%2C%22write%22%5D" \
...
**API 엔드포인트:**
- GET `/v1/admin/job-queue/stats` - 작업 큐 통계 조회
- GET `/v1/admin/job-queue/{job_id}` - 작업 상태 조회
...
**API 엔드포인트:**
- GET `/v1/admin/cache/stats` - 캐시 통계 조회
- POST `/v1/admin/cache/clear` - 캐시 초기화
...
| 0.6.0 | 2026-05-30 | 보안 가이드 초기 작성 | Cascade |
