...
**개요**: 피부 상태 정밀 분석을 위한 PCR 검사 요청 및 결과 확인을 제공합니다.

**특징**:
- PCR 검사 요청 API
- 자동 PCR 검사 키트 발송 (배송지 정보 제공 시)
- 검사 결과 조회 및 해석
- 검사 이력 관리
- 의료 전문가 상담 예약

**DB 테이블**:
- pcr_test_requests: PCR 검사 요청
- pcr_test_results: PCR 검사 결과
- pcr_consultations: 전문가 상담

---

### 3.7 기성품 구매

**개요**: 미리 준비된 5종의 기성품을 앱에서 직접 구매하는 기능을 제공합니다.

**특징**:
- 기성품 목록 조회 (재고, 가격, 카테고리)
- 기성품 주문 생성
- 주문 내역 조회
- 배송 추적

**DB 테이블**:
- products: 제품 정보 (is_ready_made 컬럼으로 기성품 구분)
...
| **DB 스키마 버전** | `src/db/skin_analysis_db.py`<br/>`schema_version` 테이블 | `40` | 데이터베이스 마이그레이션 버전 | 스키마 변경 시 |
...
**개요**: 맞춤형 화장품 추천 후 구매 프로세스를 지원하는 종합적인 주문 관리 시스템을 제공합니다.
...
**주문 상태**: pending_payment, paid, processing, shipped, delivered, cancelled
...
**자세한 가이드**: [ORDER_MANAGEMENT_GUIDE.md](guides/ORDER_MANAGEMENT_GUIDE.md)

---

## 4. 아키텍처

자세한 아키텍처 정보는 [ARCHITECTURE.md](guides/ARCHITECTURE.md)를 참조하세요.
...
│   ├── gui/                    # GUI 모듈
│   │   ├── skin_analysis_pipeline.py  # 메인 GUI
...
- [ARCHITECTURE.md](guides/ARCHITECTURE.md): Strategy 패턴 (분석기·복원기·LLM), DB 구조, 의존성 방향도
- [ORDER_MANAGEMENT_GUIDE.md](guides/ORDER_MANAGEMENT_GUIDE.md): 주문 관리 기능 가이드 (주문 생성, 결제, 배송, 재고, 통계, 알림, 바코드, 라벨)
