...
**테이블: pcr_test_requests**

| 컬럼 | 타입 | 설명 |
|------|------|------|
| id | INTEGER | Primary Key, Auto Increment |
| request_id | TEXT | 요청 ID (Unique) |
| customer_id | TEXT | 고객 ID (FK) |
| test_type | TEXT | 검사 유형 (skin_analysis, microbiome, etc.) |
...
**테이블: pcr_test_results**

| 컬럼 | 타입 | 설명 |
|------|------|------|
| id | INTEGER | Primary Key, Auto Increment |
| result_id | TEXT | 결과 ID (Unique) |
| request_id | TEXT | 요청 ID (FK) |
| customer_id | TEXT | 고객 ID (FK) |
| test_data | TEXT | 검사 데이터 (JSON) |
| interpretation | TEXT | 해석 결과 |
...
**테이블: pcr_consultations**

| 컬럼 | 타입 | 설명 |
|------|------|------|
| id | INTEGER | Primary Key, Auto Increment |
| consultation_id | TEXT | 상담 ID (Unique) |
| customer_id | TEXT | 고객 ID (FK) |
| request_id | TEXT | 요청 ID (FK) |
...
**인덱스:**
- `idx_consultation_id`: consultation_id
- `idx_customer_id`: customer_id
- `idx_request_id`: request_id
...
|------|------|------|
| id | INTEGER | Primary Key, Auto Increment |
| product_id | TEXT | 제품 ID (Unique) |
| product_name | TEXT | 제품 이름 |
| category | TEXT | 카테고리 |
| key_ingredients | TEXT | 주요 성분 (JSON) |
| efficacy | TEXT | 효능 |
| target_skin_types | TEXT | 대상 피부 타입 (JSON, nullable) |
| target_concerns | TEXT | 대상 문제점 (JSON, nullable) |
| stock_quantity | INTEGER | 재고 수량 |
| price | REAL | 제품 가격 |
| is_active | INTEGER | 활성 상태 (0: 비활성, 1: 활성) |
| is_ready_made | INTEGER | 기성품 여부 (0: 맞춤형, 1: 기성품) |
| description | TEXT | 제품 설명 (nullable) |
| created_at | TIMESTAMP | 생성 시간 |
| updated_at | TIMESTAMP | 업데이트 시간 |
...
- `idx_is_ready_made`: is_ready_made
...
Customer (고객)
  ...
  ├── 1:N → PCRTestRequest (PCR 검사 요청)
  └── 1:N → PCRConsultation (PCR 상담)
...
PCRTestRequest (PCR 검사 요청)
  ├── 1:1 → PCRTestResult (PCR 검사 결과)
  └── 1:N → PCRConsultation (PCR 상담)

Product (제품)
  ├── 1:N → OrderItem (주문 항목)
  └── (is_ready_made: 기성품 여부)
```
