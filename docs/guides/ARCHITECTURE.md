...
모든 로컬 DB 파일은 `data/` 폴더에 통합 관리됩니다.

```
data/
├── skin_analysis.db      # 피부 분석 DB (사용자, 분석 결과, 처방전 등)
├── images.db             # 이미지 메타데이터 DB
├── execution_history.db  # 실행 이력 DB
└── backups/              # 백업 파일들
...
| DB 파일 | 설명 | 주요 테이블 |
|---------|------|-----------|
| `skin_analysis.db` | 피부 분석 결과 관리 | users, analyses, prescriptions, devices, surveys, bookmarks |
