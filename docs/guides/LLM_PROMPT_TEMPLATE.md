# Gemini AI 피부 분석 프롬프트 템플릿 (LLM Prompt Template)
...
**점수 표시 정책**:
- **LLM 프롬프트**: 내부 계산용 float 값 전달 (예: {overall_score:.1f})
- **출력 JSON**: LLM 응답의 점수는 정수로 변환되어 저장 (report_to_dict 함수)
- **고객 표시**: 모든 점수는 정수로 표시됩니다 (소수점 없음)
...
**사용처**:
- GUI(skin_analysis_pipeline.py)에서 화장품 매칭 결과를 LLM에 전달
...
8. 관리 권고사항은 RECOMMENDATION_GUIDELINES 섹션의 가이드라인을 참조하여 작성하되, 반드시 JSON 배열(string 배열) 형식으로 출력하십시오.
...
**중요 지침 - 복원 이미지 기준선 활용 시 주의사항**:
- 복원 이미지는 조명·노이즈·압축 아티팩트가 제거된 깨끗한 상태이므로, 이를 기준선으로 활용할 때 원본 점수를 과대평가하지 않도록 주의하십시오.
- **특히 여드름(acne_score), 색소(melasma_score, freckle_score), 홍조(redness_score) 등의 항목**: 복원 이미지에서는 이러한 문제가 제거된 상태이므로, 복원 이미지의 깨끗한 상태를 기준으로 원본 점수를 산출하면 원본의 실제 문제가 과소평가될 수 있습니다.
- 원본 이미지에서 실제 관찰되는 피부 문제(여드름, 색소, 홍조 등)는 복원 이미지의 기준선과 무관하게 원본 이미지의 실제 상태를 기준으로 점수를 산출하십시오.
- 복원 이미지의 기준선은 주름·모공·탄력 등 구조적 항목의 참고용으로만 활용하고, 트러블·색소·홍조 등의 항목은 원본 이미지의 실제 상태를 우선적으로 반영하십시오.
- 원본 이미지와 복원 이미지의 점수 차이가 큰 항목(15점 이상 차이)은 LLM 오탐 가능성이 있으므로, 원본 이미지의 실제 상태를 더욱 정확하게 관찰하여 점수를 산출하십시오.
...
- **JSON 출력 시 반드시 모든 문자열을 큰따옴표(")로 감싸고 모든 괄호를 닫으십시오. recommendation 필드는 반드시 JSON 배열(string 배열) 형식으로 출력하십시오.**
...
### 복원 이미지 (시스템 분석기 로직)
- 종합 점수: {ref_overall_score}점 (10~90 스케일)
- 인지 나이: {ref_perceived_age}세
...
**처방 항목 설명**:
- M01: 베이스믹스 - 기본 베이스 성분
- M02: UV케어믹스 - 자외선 차단 성분
- M03: 지성믹스 (피부 타입: 지성) - 유분 조절 성분
- M04: 건성믹스 (피부 타입: 건성) - 수분 공급 성분
- M05: 복합성믹스 (피부 타입: 복합성) - 유분/수분 밸런스 성분
- M06: 민감성믹스 (피부 타입: 민감성) - 진정 성분
- M07: 보습케어믹스 - 보습 성분
- M08: 미백케어믹스 (dullness_score, melasma_score, freckle_score, post_acne_pigment_score) - 비타민C, 나이아신아마이드 성분
- M09: 탄력케어믹스 (jawline_blur_score, cheek_sagging_score) - 콜라겐, 엘라스틴 성분
- M10: 주름케어믹스 (eye_wrinkle_score, nasolabial_wrinkle_score, fine_deep_wrinkle_score) - 레티놀, 펩타이드, 아데노신 성분
- M11: 트러블케어믹스 (acne_score) - 살리실산, 티트리 오일, 나이아신아마이드 성분
- M12: 모공케어믹스 (pore_size_score, pore_sagging_score) - 살리실산, BHA, PHA 성분
- M13: 피부결케어믹스 (roughness_score) - AHA, 효소, 각질 제거 성분
- M14: 홍조케어믹스 (redness_score, post_inflammatory_erythema_score) - 시카, 판테놀, 알란토인 성분
...
- **JSON 출력 시 반드시 모든 문자열을 큰따옴표(")로 감싸고, 모든 중괄호와 대괄호를 닫으십시오. 응답이 중단되지 않도록 모든 필드를 완전히 작성하십시오. recommendation 필드는 반드시 JSON 배열(string 배열) 형식으로 출력하십시오.**

**중요 지침 - 원본 점수 산출 시 주의사항**:
- 복원 이미지는 조명·노이즈·압축 아티팩트가 제거된 깨끗한 상태이므로, 원본 점수 산출 시 복원 이미지의 깨끗한 상태에 영향을 받지 않도록 주의하십시오.
- **특히 여드름(acne_score), 색소(melasma_score, freckle_score), 홍조(redness_score) 등의 항목**: 복원 이미지에서는 이러한 문제가 제거된 상태이므로, 원본 점수 산출 시 복원 이미지의 깨끗한 상태를 참고하여 원본 점수를 과대평가하지 마십시오.
- 원본 이미지에서 실제 관찰되는 피부 문제(여드름, 색소, 홍조 등)는 복원 이미지의 상태와 무관하게 원본 이미지의 실제 상태를 기준으로 점수를 산출하십시오.
- 복원 이미지는 주름·모공·탄력 등 구조적 항목의 참고용으로만 활용하고, 트러블·색소·홍조 등의 항목은 원본 이미지의 실제 상태를 우선적으로 반영하십시오.
- 원본 이미지와 복원 이미지의 점수 차이가 큰 항목(15점 이상 차이)은 LLM 오탐 가능성이 있으므로, 원본 이미지의 실제 상태를 더욱 정확하게 관찰하여 점수를 산출하십시오.
...
| 0.1.0 | 2026-05-29 | LLM 프롬프트 템플릿 초기 작성 | Cascade |
