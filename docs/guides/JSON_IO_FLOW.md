...
**참고 문서:**
- JSON 구조 상세: `docs/db/DATA_MODEL.md`
- API 엔드포인트: `docs/api/API_REFERENCE.md`
...
| 단계 | 설명 |
|------|------|
| **1. 스마트폰 데이터 수집** | - 이미지 캡처 (정면/좌측 45°/우측 45°) 3장<br>- 설문 응답 입력 (survey)<br>- 클라이언트 메타 수집 (client_meta)<br>- 고객 정보 입력 (customer_name, customer_contact, customer_address)<br><br>↓ POST /v1/analysis/jobs<br>multipart/form-data<br>- `images[]` (JPEG, 1~3장)<br>- `angles[]` (front/left45/right45, images[]와 1:1 대응)<br>- `survey` (JSON string)<br>- `client_meta` (JSON string)<br>- `customer_id` (선택)<br>- `customer_name` (필수)<br>- `customer_contact` (필수)<br>- `customer_address` (필수) |
...
2. **피부 타입별 믹스:** `_calculate_skin_type_mix()` 호출
   - oily → M03 (유분, 2%)
   - dry → M09 (수분, 2%)
   - combination → M03 + M09 (유분 + 수분, 1% + 1%)
   - sensitive → M09 (수분, 2%)
3. **고민사항별 믹스:** `_calculate_concern_mix()` 호출
   - 색소 (melasma, freckle, pigmentation) → M05 (색소침착, 2%)
   - 홍조 (redness, red_marks) → M06 (홍조, 2%)
   - 트러블 (acne, pimple) → M10 (트러블, 2%)
   - 모공 (pore, pore_sebum, pore_care) → M07 (모공, 2%)
   - 주름 (wrinkle, eye_wrinkle, nasolabial) → M02 (주름, 2%)
   - 텍스처 (texture, roughness) → M08 (피부결, 2%)
   - 톤 (tone, dullness, uneven_tone) → M01 (톤&밝기, 2%)
   - 탄력 (elasticity, sagging, jawline) → M04 (탄력&처짐, 2%)
   - 노화 (aging) → M02 + M04 (주름 + 탄력, 1% + 1%)
4. **피부 평가 기반 처방:** `calculate_skin_assessment_recipe()` 호출
   - 측정항목 점수 → 믹스 코드 매핑 (config.json의 measurement_settings.mix_codes)
   - 가장 낮은 점수 기준으로 처방 비율 결정
   - 점수 기준: good_threshold(76) 이상 0%, critical_threshold(40) 미만 max_percentage(3%)
   - [REFACTOR 2026-06-08] 처방전 객체에 믹스 코드 한국어 이름 포함 (SSOT 원칙)
5. **PCR 기반 처방:** `calculate_pcr_recipe()` 호출
   - PCR 규칙 적용 (PM01-PM07)
   - [REFACTOR 2026-06-08] PCR 믹스에도 한국어 이름 포함
...
```json
{
  "base": {"percentage": 85},
  "skin": {
    "M03": {"percentage": 1, "name": "유분"},
    "M09": {"percentage": 1, "name": "수분"}
  },
  "care": {
    "M10": {"percentage": 2, "name": "트러블"},
    "M06": {"percentage": 2, "name": "홍조"},
    "M07": {"percentage": 2, "name": "모공"}
  },
  "pcr": {
    "PM01": {"percentage": 1.5, "name": "프로바이오틱 믹스"},
    "PM02": {"percentage": 1.5, "name": "프리바이오틱 믹스"}
  },
  "assessment": {
    "M01": {"percentage": 1, "name": "톤&밝기"},
    "M02": {"percentage": 2, "name": "주름"},
    "M05": {"percentage": 1, "name": "색소침착"}
  }
}
```
...
**[REFACTOR 2026-06-08] 믹스 코드 이름 매핑:**
- 처방전 생성 시 config.json의 `measurement_settings.mix_codes`에서 한국어 이름 로드
- 처방전 객체에 이름 직접 포함 (SSOT 원칙)
- GUI/HTML/Excel 보고서 표시 시 config 조회 불필요
...
| `customer_name` | text | 예 | 고객 이름 (필수) |
| `customer_contact` | text | 예 | 고객 연락처 (필수) |
| `customer_address` | text | 예 | 고객 주소 (필수) |
...
| 문서 버전 | 날짜 | 변경 내용 | 작성자 |
|-----------|------|----------|--------|
| 1.2.0 | 2026-06-10 | 폴더 구조 설명 수정 ({customer_id} → {stem}, stem 동작 추가), text2img 모드 제거 관련 문서 갱신 | Cascade |
