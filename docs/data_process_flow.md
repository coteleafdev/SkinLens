...
| **7. 처방 계산** | - PrescriptionCalculator로 처방전 생성<br>- AGE_GROUP_MAPPING으로 나이대 그룹 매핑 (config.json에서 로드)<br>- PCR (Prescription Calculator Rules) 규칙 적용<br>- total/beneficial/trouble/harmful 계산<br>- 믹스 코드 계산 (M01~M14)<br>- base 비율 계산 (중복 제거)<br>- 처방 항목 생성 |
