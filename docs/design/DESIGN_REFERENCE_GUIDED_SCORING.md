# 복원 이미지 기반 원본 점수 정확도 향상 설계 (Design Reference Guided Scoring)
...
generate_dual_report()
    ...
    │     Step 3. 최종 원본 점수 산출
    │
    ├── _call_llm([orig, ref])          ← 이미지 순서 고정: 원본→복원
    │
...
## 8. 최신 변경 사항 (2026-06-07)

### 8.1 CV 점수 모니터링 개선 (provide_scores=False 지원)
...
- **구현**:
  ...
  - `generate_reference_guided_report()`에 CV 점수 모니터링 매개변수 추가 (`orig_cv_measurements`, `ref_cv_measurements`, `orig_cv_overall`, `ref_cv_overall`, `orig_cv_age`, `ref_cv_age`)
  - RGP 모드에서 저장된 CV 점수를 사용하여 점수 차이 모니터링

### 8.2 점수 기준 섹션 추가 (2026-05-28)
...
| 문서 버전 | 날짜 | 변경 내용 | 작성자 |
|-----------|------|----------|--------|
| 1.1.0 | 2026-06-07 | CV 점수 모니터링 개선 (provide_scores=False 지원) | Cascade |
| 1.0.0 | 2026-05-31 | 초기 버전 (v1.0에서 마이그레이션) | Cascade |
| 0.1.0 | 2026-05-27 | 복원 이미지 기반 원본 점수 정확도 향상 설계 문서 초기 작성 | Cascade |
