...
> **⚠️ 최상위 문서 (Top-Level Document)**: 이 문서는 SkinLens REST API 엔드포인트 상세 설명을 제공하는 최상위 문서입니다. 모든 개발자가 참조해야 합니다.
> 
> **관련 최상위 문서**: 
...
**참고 문서:**
- 전체 시스템 아키텍처: `docs/guides/ARCHITECTURE.md`
...
| 주문 API | `/v1/orders/*` | 웹, 모바일 | 필수 | 주문 생성, 결제, 배송, 피드백, 기성품 목록 |
| 앱 기능 API | `/v1/app/*` | 웹, 모바일 | 필수 | 피부 일기, 목표, 업적, 구독, 챌린지, PCR 검사 |
...
| customer_name | string | 예 | 고객 이름 (필수) |
| customer_contact | string | 예 | 고객 연락처 (필수) |
| customer_address | string | 예 | 고객 주소 (필수) |
...
| 필드 | 타입 | 필수 | 설명 |
|------|------|------|------|
| image_type | string | 아니오 | 이미지 타입 (restored 또는 input, 기본: restored) |

**Response (200 OK):** 이미지 파일 (PNG)
...
PCR 검사를 요청합니다. 배송지 정보를 제공하면 PCR 검사 키트가 자동으로 발송됩니다.
...
**GET** `/v1/app/pcr/results/{customer_id}`

고객의 PCR 검사 결과를 조회합니다.
...
**GET** `/v1/app/pcr/history/{customer_id}`

고객의 PCR 검사 이력을 조회합니다.
...
**GET** `/v1/app/pcr/consultations/{customer_id}`
...
| 0.1.0 | 2026-05-30 | API 레퍼런스 초기 작성 | Cascade |
