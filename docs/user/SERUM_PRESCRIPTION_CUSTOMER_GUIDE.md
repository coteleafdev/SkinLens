...
- **출력 JSON**: 모든 점수는 정수로 표시됩니다 (소수점 없음)
  - 피부 평가 점수 (18개 항목): 정수 (예: 56, 72, 68)
  - 종합 점수: 정수 (예: 65)
- **내부 계산**: float로 유지 (정밀도 보장)
- **처방 비율 계산**: 내부적으로 float로 계산되나, 최종 출력은 정수 점수 기반
...
- **활성 믹스 10종 (M01-M10)**: 피부 평가 점수 기반 처방
  - **현재 사용 중인 믹스 10종**: 톤&밝기(M01), 주름(M02), 유분(M03), 탄력&처짐(M04), 색소침착(M05), 홍조(M06), 모공(M07), 피부결(M08), 수분(M09), 트러블(M10)
...
**구현 상태**: ✅ 현재 구현됨
- `survey["skin_types"][0]`에서 피부 타입 추출
- `survey["skin_concerns"]`에서 고민사항 추출
- 추출된 정보는 처방전 계산에 자동 반영됩니다
...
**구현 상태**: ✅ 현재 구현됨
- 함수: `_calculate_skin_type_mix(skin_type)`
- 데이터 소스: `survey["skin_types"][0]`
- 처방 비율: 각 피부타입별 2.0% (복합성은 1.0% + 1.0%)

### 피부타입별 믹스 상세

| 피부타입 | 세럼 원료 | 믹스 코드 | 주요 성분 | 어떻게 작용하나요? | 처방 비율 |
|---------|---------|---------|---------|------------------|------------|
| **지성** | 지성용 믹스 | M03 | 니아신아마이드, 레스베라트롤, 티트리 오일 | 피지 과다 분비를 조절하고 모공을 수축하며 항균 작용으로 트러블 예방 | 2% |
| **건성** | 건성용 믹스 | M09 | 히알루론산, 세라마이드, 스쿠알란 | 피부에 수분을 공급하고 보습막을 형성하여 피부 장벽을 강화 | 2% |
| **복합성** | 복합용 믹스 | M03 + M09 | 판테놀, 알란토인, 글리콜산 | 유분과 수분 밸런스를 조절하고 피부를 진정하며 부드럽게 각질 제거 | 2% (M03 1% + M09 1%) |
| **민감성** | 민감용 믹스 | M09 | 시카 추출물, 알로에 베라, 비타민 E | 민감한 피부를 진정하고 항염 작용으로 피부를 보호 | 2% |
...
**구현 상태**: ✅ 현재 구현됨
- 함수: `_calculate_concern_mix(concerns)`
- 데이터 소스: `survey["skin_concerns"]`
- 처방 비율: 각 고민사항별 2.0% (노화는 1.5% + 1.5%)
- 지원 고민사항: 색소, 홍조, 트러블, 모공, 주름, 텍스처, 톤, 탄력, 노화

### 관심별 믹스 상세

| 관심 분야 | 세럼 원료 | 믹스 코드 | 주요 성분 | 어떻게 작용하나요? | 설문 처방 % |
|---------|---------|---------|---------|------------------|------------|
| **색소 (Pigmentation)** | 색소 케어 믹스 | M05 | 비타민 C, 나이아신아마이드, 알부틴 | 멜라닌 생성을 억제하고 기존 색소를 밝게 하여 톤을 개선 | 2.0% |
| **홍조·혈관 (Redness & Vascular)** | 홍조 케어 믹스 | M06 | 시카 추출물, 알란토인, 비타민 K | 염증을 진정하고 혈관을 강화하여 홍조를 완화 | 2.0% |
| **모공 (Pore Analysis)** | 모공 케어 믹스 | M07 | 살리실산, 레티놀, 티트리 오일 | 모공을 수축하고 피지 조절을 도우며 각질을 제거하여 모공 개선 | 2.0% |
| **주름 (Wrinkle & Line)** | 주름 케어 믹스 | M02 | 레티놀, 펩타이드, 아데노신 | 콜라겐 생성을 촉진하고 주름을 개선하며 탄력을 증진 | 2.0% |
| **피부결 (Texture)** | 피부결 케어 믹스 | M08 | AHA, BHA, 효소 | 묵은 각질을 제거하고 피부결을 매끄럽게 개선 | 2.0% |
| **톤 & 밝기 (Tone & Brightness)** | 피부톤 케어 믹스 | M01 | 비타민 C, 글루타치온, 리포산 | 피부 톤을 밝게 하고 광채를 주며 항산화 작용 | 2.0% |
| **탄력 & 처짐 (Elasticity & Sagging)** | 탄력 케어 믹스 | M04 | 펩타이드, 콜라겐, 엘라스틴 | 피부 탄력을 증진하고 처짐을 개선하며 피부 리프팅 효과 | 2.0% |
| **노화 (Aging)** | 노화케어 믹스 | M02 + M04 | 코엔자임 Q10, 레스베라트롤, 비타민 E | 항산화 작용으로 노화를 방지하고 세포를 보호 | 3.0% (M02 1.5% + M04 1.5%) |
| **트러블 & 흔적 (Acne & Marks)** | 트러블케어 믹스 | M10 | 살리실산, 티트리 오일, 과산화벤조일 | 항균 작용으로 염증을 완화하고 트러블을 예방 | 2.0% |
...
3. **설문 기반 (현재 구현됨)**: 사용자가 선택한 피부타입과 관심사에 따라 기본 비율 적용
   - 피부타입별 믹스: `_calculate_skin_type_mix()` 함수로 계산
   - 고민사항별 믹스: `_calculate_concern_mix()` 함수로 계산
...
|---------|---------|---------|------|--------------|
| M01 | 톤&밝기 케어 믹스 | radiance | 톤 & 밝기 관리 | ✅ 사용 중 |
| M02 | 주름 케어 믹스 | wrinkle | 주름 관리 | ✅ 사용 중 |
| M03 | 유분 믹스 | oiliness | 유분 관리 | ✅ 사용 중 (피부 타입 기반) |
| M04 | 탄력&처짐 케어 믹스 | firmness | 탄력 & 처짐 관리 | ✅ 사용 중 |
| M05 | 색소 케어 믹스 | age_spot | 색소침착 관리 | ✅ 사용 중 |
| M06 | 홍조 케어 믹스 | redness | 홍조·혈관 관리 | ✅ 사용 중 |
| M07 | 모공 케어 믹스 | pore | 모공 관리 | ✅ 사용 중 |
| M08 | 피부결 케어 믹스 | texture | 피부결 관리 | ✅ 사용 중 |
| M09 | 수분 믹스 | moisture | 수분 관리 | ✅ 사용 중 (피부 타입 기반) |
| M10 | 트러블 케어 믹스 | acne | 여드름 & 흔적 관리 | ✅ 사용 중 |

### 현재 사용 중인 활성 믹스 10종

- **M01**: 톤&밝기 케어 믹스 (톤 & 밝기)
- **M02**: 주름 케어 믹스 (주름)
- **M03**: 유분 믹스 (유분 관리 - 피부 타입 기반)
- **M04**: 탄력&처짐 케어 믹스 (탄력 & 처짐)
- **M05**: 색소 케어 믹스 (색소침착)
- **M06**: 홍조 케어 믹스 (홍조·혈관)
- **M07**: 모공 케어 믹스 (모공)
- **M08**: 피부결 케어 믹스 (피부결)
- **M09**: 수분 믹스 (수분 관리 - 피부 타입 기반)
- **M10**: 트러블 케어 믹스 (여드름 & 흔적)
...
- **처방 항목**: 피부 상태 분석 결과 기반 처방 (예: M01 톤&밝기, M06 색소침착)
...
- `docs/PRESCRIPTION_MAPPING.md`: 18개 측정항목 → 13개 처방 항목 매핑
